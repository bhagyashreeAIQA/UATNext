 ...
 * Test Case ID : TC-120
 * Test Case Name: Verify Reason Field Value Selection in Create Defect Screen
 *
 * Description  : As a Test Engineer, I want to verify that I can view and select values from the
 *                Reason dropdown in the Create Defect screen.
 *
 * Pre-conditions:
 ...
 * Steps:
 *   1. Follow TC-098.
 *   2. Click the Reason dropdown.
 *   3. Select a Reason value.
 *
 * Expected:
 *   1. Create Defect page/panel opens.
 *   2. Reason values are displayed.
 *   3. Selected Reason value is displayed and retained.
 *
 * Note: Reason has an empty default; a value is selected. No defect is created.
 */
...
const PLACEHOLDER = EXPECTED.createDefect.dropdownPlaceholders.reason;

test.describe('Feature: Execute Test Case | Sub-Feature: Test Run Execution Details – Create Defect', () => {

  test('TC-120 | Verify Reason Field Value Selection in Create Defect Screen', async ({ page }) => {
    ...
    // ─── Step 2 / Expected 2: the Reason dropdown lists values ───────────────────
    expect(await executionPage.getDefectDropdownOptionCount(PLACEHOLDER),
      'Reason dropdown should list values').toBeGreaterThan(0);
    await captureScreenshot(page, "Step 2 / Expected 2: the Reason dropdown lists values");
