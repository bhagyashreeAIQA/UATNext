...
test.describe('Feature: Defect | Sub-Feature: Defect Search', () => {
  ...
  test('Def_TC_015 | Verify Defect Search by Invalid Severity', async ({ page }) => {
    ...
    await captureScreenshot(page, "Step 3: Enter an invalid Severity value in the Severity dropdown");
    ...
    await captureScreenshot(page, "Step 4: No valid severity is committed → no severity filter is applied");
  });
