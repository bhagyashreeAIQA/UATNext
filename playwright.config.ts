...
export default defineConfig({
  ...
  // One retry absorbs transient races against the shared dev backend (occasional slow
  ...
  retries: 1,
  // Cap local workers: the suite drives a shared backend (Blazor/SignalR streaming) in
  // headed mode, and too many concurrent browsers starve it (sidebar stalls on "Loading
  // projects...", page loads exceeding the 30s default test timeout). Verified live (2026-08-04):
  // a batch of 19 tests that failed/flaked at workers=6 against stg passed 100% clean at
  // workers=2 — two keeps the run parallel but stable.
  ...
  use: {
    baseURL: 'https://webapp-v1-blazor-uatnext-stg.azurewebsites.net',
